...
class SOM:
    def __init__(self, X, X_after, latent_dim, resolution, sigma_max, sigma_min, tau, init='random',metric="sqeuclidean"):
        self.X = X
...
#            bar = tqdm(range(nb_epoch))
    ...
    def ploof(self):
        ...
        self.Y_ploof = self.Y[:, 0:check_dim]
        self.X_after = np.array(self.X_after)
        self.X_after_ploof = self.X_after[:, :, 0:check_dim]
        print('01', self.X_after.shape)
        print('01', self.X_after_ploof.shape)
        print('02', self.Y_ploof.shape)
        Y_result = np.zeros((100, 7))

        for n in range(100):
            Dist = dist.cdist(self.X_after_ploof[:, n, :], self.Y_ploof)  # NxKの距離行列を計算
            bmus = Dist.argmin(axis=1)
            Y_result[n, :] = self.Y[bmus, :]

        s = np.hstack((Y_result, self.X_after[0, :, :]))
        print('Ploof fin')

        np.savetxt('SOM_result.csv', s, delimiter=',', header='BMU_elements+Answer', comments='#comments#')
